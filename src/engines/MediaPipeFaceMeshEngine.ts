...
interface Sample { features: number[]; sx: number; sy: number; }

declare global {
  interface Window {
    FaceMesh: new (cfg: { locateFile: (file: string) => string }) => FaceMeshAPI;
  }
}
interface FaceMeshAPI {
  ...
  onResults(cb: (r: { multiFaceLandmarks?: Lm[][] }) => void): void;
...
function ridgeFit(X: number[][], y: number[], lam = 1e-3): number[] {
  const F = X[0].length;
  const XtX = Array.from({ length: F }, (_, i) =>
    Array.from({ length: F }, (_, j) =>
      X.reduce((s, row) => s + row[i] * row[j], 0) + (i === j ? lam : 0)
    )
  );
  const Xty = Array.from({ length: F }, (_, i) =>
...
function polyFeatures(f: number[]): number[] {
...
export class MediaPipeFaceMeshEngine implements GazeEngine {
  private gazeListener: ((pt: GazePoint) => void) | null = null;
  private running = false;
  private fm: FaceMeshAPI | null = null;
  ...
  async stop(): Promise<void> {
    ...
    this.smoothX = null;
    this.smoothY = null;
    if (this.fm) { try { await this.fm.close(); } catch { /* ignore */ } this.fm = null; }
    if (this.stream) { this.stream.getTracks().forEach(t => t.stop()); this.stream = null; }
    if (this.video)  { this.video.remove(); this.video = null; }
  }

  setGazeListener(cb: (pt: GazePoint) => void): void { this.gazeListener = cb; }
  clearGazeListener(): void { this.gazeListener = null; }

  train(x: number, y: number): void {
    const count = Math.min(this.frameBuffer.length, TRAIN_FRAMES);
    if (count === 0) return; // camera not ready yet

    // Average the most recent frames → one stable feature vector per click
    const F = this.frameBuffer[0].length;
    const avg = new Array<number>(F).fill(0);
    const start = this.frameBuffer.length - count;
    for (let i = start; i < this.frameBuffer.length; i++) {
      for (let j = 0; j < F; j++) avg[j] += this.frameBuffer[i][j];
    }
    for (let j = 0; j < F; j++) avg[j] /= count;
    ...
    if (this.training.length >= 6) this._fit();
    this._persist();
  }
  ...
  private async _initFaceMesh(): Promise<void> {
    ...
    fm.onResults(r => {
      ...
      if (this.frameBuffer.length > BUFFER_SIZE) this.frameBuffer.shift();
  ...
  private _runLoop(): void {
    ...
    const tick = async () => {
      if (!this.loopActive || !this.fm || !this.video) return;
      if (this.video.readyState >= 2) await this.fm.send({ image: this.video });
      requestAnimationFrame(tick);
    };
    requestAnimationFrame(tick);
  }

  private _features(lm: Lm[]): number[] | null {
  ...
  private _eyeFraction(
    lm: Lm[],
    irisIdx: number,
    e: { outer: number; inner: number; top: number; bottom: number },
  ...
  private _restoreModel(): void {
    try {
      const raw = localStorage.getItem(STORE_KEY);
      if (!raw) return;
      const d = JSON.parse(raw) as { training: Sample[]; wx: number[]; wy: number[] };
